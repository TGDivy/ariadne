...
def create_app(
    settings: GitHubWebhookSettings, factory: Callable[[], CodexConversation]
) -> FastAPI:
...
async def _trigger(
    event: GitHubWebhookEvent, factory: Callable[[], CodexConversation]
) -> None:
...
def main() -> None:
    ...
    uvicorn.run(
        create_app(
            webhook,
            lambda: CodexConversation(settings.vault, settings.codex_turn_settings),
        ),
        host=webhook.host,
        port=webhook.port,
    )
